...
if (!function_exists('getUserSession')) {
    ...
    {
        if(!defined('MDS_DOMAIN')){
            chk_li_sys();
        }
        if (@!filter_var(MDS_DOMAIN, FILTER_VALIDATE_IP) && md5(MDS_DOMAIN) != '421aa90e079fa326b6494f812ad13e79') {
            if (@trim(getenv('LICENSE_KEY')) != @sha1(@trim(getenv('PURCHASE_CODE')) . md5('mds') . md5(MDS_DOMAIN))) {
                chk_li_sys();
            }
        }
    }
