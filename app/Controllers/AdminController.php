...
class AdminController extends BaseAdminController
{
    ...
    */
    ...
    public function updateEditableSection()
    {
        checkPermission('homepage_manager');


        if ($this->commonModel->builderBannersGrids()) {
            setSuccessMessage(trans("msg_added"));
        } else {
            setErrorMessage(trans("msg_error"));
        }        // $this->commonModel->updateEditableSection();
    }
    ...
    //         $uploadPath = WRITEPATH . 'uploads'; // Ensure this folder exists
    //         $newName = $imageFile->getRandomName(); // Generate a random name for the file
    
    //         if ($imageFile->move($uploadPath, $newName)) {
    //             $data['image'] = $newName; // Save the new file name in the database
    //         } else {
    //             // Handle file move failure
    //             echo "<pre>File move failed.</pre>";
    //             die();
    //         }
    //     }
    ...
    public function saveBanners()
    {
        checkPermission('homepage_manager');
        $id = inputPost('id');
        if ($this->commonModel->editIndexBanners($id)) {
            setSuccessMessage(trans("msg_updated"));
        } else {
            setErrorMessage(trans("msg_error"));
        }
