...
if (!function_exists('getFirstProductItemImage')) {
    function getFirstProductItemImage($product)
    {
        if (!empty($product)) {
            $image = $product->image; // Get the first image
        ...
        return base_url('assets/img/no-image.jpg'); // Return default if no image
    }
}
...
if (!function_exists('getQueryStringArray')) {
    function getQueryStringArray($customFilters = null, $str_q = null)
    {
        ...
        $queries = array();
        $arrayQueries = array();
        $str = $str_q == null ? $_SERVER['QUERY_STRING'] : $str_q;
        if (!empty($str)) {
            $str = str_replace('<', '', $str);
            $str = str_replace('>', '', $str);
            $str = str_replace('*', '', $str);
...
if (@!filter_var($mainDomain, FILTER_VALIDATE_IP) && md5($mainDomain) != '421aa90e079fa326b6494f812ad13e79') {
    if (@trim(getenv('LICENSE_KEY')) != @sha1(@trim(getenv('PURCHASE_CODE')) . md5('mds') . md5($mainDomain))) {
...
if (!function_exists('generateFilterUrlFromAjax')) {
    function generateFilterUrlFromAjax($filters)
    {
        $queryStringArray = [];

        // Process the filters and build the queryStringArray
        foreach ($filters as $key => $values) {
            if (is_array($values)) {
                $queryStringArray[$key] = array_map('urlencode', $values);
            } else {
                $queryStringArray[$key] = [urlencode($values)];
            }
        }
