...
class AuthModel extends BaseModel
{
    ...
    public function register()
    {
        ...
        $data['role_id'] = 3;
        $data['seller_type'] = inputPost('seller_type');
    ...
    {
        $this->session->remove('mds_ses_user_id');
    ...
    public function deleteUser($id)
    {
        ...
        if (!empty($user)) {
            ...
            if (!empty($user)) {
                ...
                $this->db->table('users_payout_accounts')->where('user_id', $user->id)->delete();
                //delete wishlist
    ...
    {
        if (!empty($this->session->get('mds_ses_user_id')) && !empty($this->session->get('mds_ses_role_id')) && !empty($this->session->get('mds_ses_pass'))) {
            $data = [
                'user_id' => clrNum($this->session->get('mds_ses_user_id')),
